...
const AdminBlog = () => {
  ...
  const [form, setForm] = useState({
    title: "", slug: "", excerpt: "", content: "", cover_image_url: "", author_name: "SOMI Team", published: false,
  });
  ...
  const openEdit = (post: BlogPost) => {
    ...
    setForm({
      ...
      author_name: post.author_name, published: post.published,
    });
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      author_name: form.author_name.trim() || "SOMI Team", published: form.published,
    ...
    setSaving(false);
    if (error) { toast({ title: "Error saving post", description: error.message, variant: "destructive" }); }
    else { toast({ title: editing ? "Post updated" : "Post created" }); setDialogOpen(false); fetchPosts(); }
  };
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <p className="text-muted-foreground text-sm mb-6">📝 <strong>Appears on:</strong> Stories page → Blog section at bottom</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <button onClick={openNew} className="inline-flex items-center gap-2 bg-primary text-primary-foreground text-sm font-semibold px-4 py-2 rounded-lg">
          ...
          {posts.map((post) => (
            <div key={post.id} className="bg-background rounded-xl border border-border p-5 flex items-center justify-between gap-4">
              <div className="flex items-center gap-4 flex-1 min-w-0">
                {post.cover_image_url && <img src={post.cover_image_url} alt="" className="w-14 h-14 rounded-lg object-cover border border-border shrink-0" />}
        ...
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-2">
            <div>
              ...
              <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value, slug: form.slug || generateSlug(e.target.value) })} className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="Post title" />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground mb-1 block">Slug</label>
              <input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="post-url-slug" />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground mb-1 block">Excerpt</label>
              <textarea value={form.excerpt} onChange={(e) => setForm({ ...form, excerpt: e.target.value })} rows={2} className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none" placeholder="Short summary..." />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground mb-1 block">Content *</label>
              <textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} rows={10} className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-y" placeholder="Write your blog post content here..." />
            </div>
            ...
            <div>
              ...
              <input value={form.author_name} onChange={(e) => setForm({ ...form, author_name: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="SOMI Team" />
            </div>
